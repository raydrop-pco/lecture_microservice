...
Session 1 outcomes:
- Explain why timeouts/retries can create duplicates without idempotency.
- Design deterministic replay behavior for create APIs.
- Build async workflows that tolerate duplicates and out-of-order events.
...
- Enforce a **unique constraint** on the same scope in storage.
...
- Use transactional insert/update flow:
  - insert `IN_PROGRESS` first,
  - execute side effects,
  - mark `SUCCEEDED` with stable `order_id`.
- Pick a realistic **TTL** (late retries happen) and document it.
- For crash recovery, reconcile stale `IN_PROGRESS` records and settle to `SUCCEEDED` or `FAILED` deterministically.
...
Status endpoint example: `GET /orders/{order_id}` returns `{ status, last_updated_at, failure_reason? }`.
...
- Keep user messaging explicit: "processing", "completed", or "failed with reason" so eventual consistency is visible, not confusing.
